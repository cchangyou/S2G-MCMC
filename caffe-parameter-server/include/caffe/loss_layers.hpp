...
template <typename Dtype>
class AveAccuracyLayer : public AccuracyLayer<Dtype> {
 public:
  /**
   * @param param provides AccuracyParameter accuracy_param,
   *     with AccuracyLayer options:
   ...
   *     correct if the correct label is among the top 5 predicted labels.
   */
  explicit AveAccuracyLayer(const LayerParameter& param)
  ...
  virtual inline const char* type() const { return "Ave Accuracy"; }
  virtual inline int ExactNumBottomBlobs() const { return 2; }
 ...
 protected:
  ...
  vector< vector<Dtype> > hist_bottom_data;
  int hist_idx;
  int burnin;
  int niter;
...
template <typename Dtype>
class SoftmaxWithLossBDKLayer : public SoftmaxWithLossLayer<Dtype> {
 public:
   /**
    * @param param provides LossParameter loss_param, with options:
    *  - ignore_label (optional)
    ...
    *    present; otherwise the loss is simply summed over spatial locations.
    */
  explicit SoftmaxWithLossBDKLayer(const LayerParameter& param)
  ...
  virtual inline const char* type() const { return "SoftmaxWithLossBDK"; }
 ...
 protected:
  ...
  /// The internal SoftmaxLayer used to map predictions to a distribution.
  shared_ptr<Layer<Dtype> > softmax_layer_1_;
  /// prob stores the output probability predictions from the SoftmaxLayer.
  Blob<Dtype> prob_1_;
  /// bottom vector holder used in call to the underlying SoftmaxLayer::Forward
  vector<Blob<Dtype>*> softmax_bottom_vec_1_;
  /// top vector holder used in call to the underlying SoftmaxLayer::Forward
  vector<Blob<Dtype>*> softmax_top_vec_1_;
