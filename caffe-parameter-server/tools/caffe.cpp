...
DEFINE_bool(locking, true,
        "whether lock parameter on asyncronous update");
DEFINE_int32(gpu, -1,
...
int train() {
    ...
    if (Caffe::GetParamServer(float(0.0)) -> rank() != 0)
    {
        // disable logging and snapshot for worker process
        solver_param.set_snapshot_after_train(false);
        solver_param.set_snapshot(false);
        solver_param.set_test_interval(0);//(1000000); //turn off testing
    ...
    Caffe::GetParamServer(float(0.0)) -> set_params(solver -> net() -> params());
    Caffe::GetParamServer(float(0.0)) -> listen(FLAGS_locking);

    // to make sure all workers start from the same point
    for (int i = 0; i < solver -> net() -> params().size(); ++i)
        Caffe::GetParamServer(float(0.0)) -> update(i);
...
LOG(INFO) << argv[i];}
    // debug
    Caffe::GetParamServer(float(0.0)) -> init(&argc, &argv);
...
LOG(INFO) << "333";
    ...
    Caffe::GetParamServer(float(0.0)) -> sync();
}
