...
void Solver<Dtype>::Init(const SolverParameter& param) {
  ...
  current_step_ = 0;
	...
	//ccy
	//lock_p.reset(new boost::mutex[1]);
	//ccy
}
...
void Solver<Dtype>::InitTestNets() {
  ...
  if (num_test_net_instances) {
    //turn off this because test_interval for rank > 0 has been set to 0
...
void Solver<Dtype>::Step(int iters) {
  ...
  for (; iter_ < stop_iter; ++iter_) {
	...
	if(Caffe::GetParamServer(Dtype(0.0))->rank() > 0){
...
template <typename Dtype>
void PSGLDSolver<Dtype>::ComputeUpdateValue() {
	const vector<shared_ptr<Blob<Dtype> > >& net_params = this->net_->params();
	const vector<float>& net_params_lr = this->net_->params_lr();
	const vector<float>& net_params_weight_decay =
      		this->net_->params_weight_decay();
...
template <typename Dtype>
void SGDBDKSolver<Dtype>::ComputeUpdateValue() {
